...
-- Final total(s)
-- Group suppression of control field
-- Displaying control fields with control break total line

CREATE OR REPLACE PROCEDURE order_listing_4 IS
  ...
  v_company_total   v_customer_total%TYPE := 0;
...
PROCEDURE L1_break IS                            -- Customer id break
  BEGIN
    DBMS_OUTPUT.PUT_LINE(LPAD('Customer ' ||
                         hold_customer_id ||
						 ' total: ' || TO_CHAR(v_customer_total, 'fm9,999,999') || ' *', 52));
    IF NOT orders_cursor%NOTFOUND THEN
      DBMS_OUTPUT.NEW_LINE;
      v_company_total := v_company_total + v_customer_total; 
...
PROCEDURE termination IS
  BEGIN
    L1_break;
    DBMS_OUTPUT.NEW_LINE;
    DBMS_OUTPUT.PUT_LINE(LPAD('Total company: ' ||
                         TO_CHAR(v_company_total, 'fm9,999,999') || ' ****', 55));
    CLOSE orders_cursor;
  END termination;
